...
& \sim N\bigg(\big(\sigma^{-2}_y \sum_{i=1}^n {x_i^2}\big)^{-1} \times \big(\sigma^{-2}_y\sum_{n=1}^n {x_i}{y_i}\big)^{-1}, \big(\sigma^{-2}_y \sum_{i=1}^n {x_i^2}\big)^{-1} \bigg)
\end{aligned}
\]
...
\newline
Posterior Distribution: $p(\tau_y|y) \propto p(y|\tau_y)p(\tau_y)$
